	...
	initComponent : function() {
		...
		this.startDayField = this.startDayField
				|| Ext.create('Ext.form.DateField', {
							fieldLabel : lan['startDayFieldN.label'],
							value : new Date(),
							format : 'Y-m-d',
							allowBlank : false,
							flex : 3,
...
//							labelStyle : 'text-align:center;',
							fieldLabel : lan['endDayFieldN.label'],
//							labelSeparator : '',
							format : 'Y-m-d',
							value : new Date(),
							allowBlank : false,
...
//							hideLabel : true,
//							labelSeparator : '',
							fieldLabel : lan['wholeField.label']
						});
...
//							labelSeparator : '',
		...
		this.timepanel1 = this.timepanel || Ext.create('Ext.Container', {
					layout : {
						type : 'hbox'
					},
		...
		this.timepanel2 = this.timepanel || Ext.create('Ext.Container', {
					layout : {
						type : 'hbox'
					},
					style : 'overflow:hidden;padding-bottom:3px;',
		...
		this.timepanel3 = this.timepanel || Ext.create('Ext.Container', {
					layout : {
						type : 'hbox'
					},
					style : 'overflow:hidden;padding-bottom:3px;',
	...
	onSaveFn : function() {
		...
		if (this.generalForm.getForm().isValid()
			...
			if (this.bindEl) {
				...
				if(!event.calendarId){
			    		Ext.Msg.alert('提示','请选择日程类别',function() {}, this );
	            			return ;
	            		}
	...
	setup : function(obj) {
		...
		this.repeatForm.doLayout();
		//新建时提示
		if("prepare"==bindEvent.eventId) {
			if(this.calendarField.store.getCount() <= 0) {
						Ext.Msg.alert('提示','请先“显示”左侧的日程分类再重新打开“新建日程”',
			            function() {},
			            this
		            	);
			}
...
Ext.define('Ext.ux.calendar.AlertSetting', {
			...
			initComponent : function() {
				...
				this.alertEarlyField = this.alertEarlyField
						|| Ext.create('Ext.form.Label', {
									style : 'margin-left:300px;line-height:22px;',
									html : lan['alertEarly.label'],
									flex : .6
								});

				this.alertpanel1 = this.alertpanel1 || Ext.create('Ext.Container', {
							layout : {
								type : 'hbox'
							},
							items : [
				...
				this.alertpanel2 = this.alertpanel2 || Ext.create('Ext.Container', {
//							layout : {
//								type : 'hbox'
//							},
