...
Ext.define('Ext.ux.calendar.view.MonthView', {
	...
	alignDetailCt : function() {
		if (this.detailing) {
			...
			var tfh = this.detailTitle.getHeight() + 20;
...
//		this.detailFoot.dom.innerHTML = events.length + ' ' + lan['events'];
	...
	_onPortDblclickFn : function(e) {
		...
		if (tgEl) {
			if (tgEl && (eh.isEditable(tgEl.bindEvent)||tgEl.bindEvent.lockedf)) { // modify by lij  允许用户查看
	...
	_onPortClickFn : function(e) {
		...
		if (Ext.ux.calendar.Mask.includeIsClass(target.className,
			...
			//处理点击“更多”时，day计算JS错误问题
			//if(isNaN(pos.x) || isNaN(pos.y)) {
			//	pos = this.getCellIndex(tgEl.dom.id);
			//}
			...
			if (Ext.ux.calendar.Mask.includeIsClass(target.className,
				...
				if (titleEl) {
					...
					if (wEl && (eh.isEditable(wEl.bindEvent)||wEl.bindEvent.lockedf)) { //modify by lij 锁定时允许用户查看
						 this.fireEvent('canceldetail'); //点击查看前隐藏列表 add by lij
					...
					if (titleEl) {
						...
						if (lEl && (eh.isEditable(lEl.bindEvent)||lEl.bindEvent.lockedf)) { //modify by lij 锁定时允许用户查看
							this.fireEvent('canceldetail'); //点击查看前隐藏列表 add by lij
...
//		x-dayview-header-days 隐藏展示
		css = this.id + '-x-monthview-viewer-col-6 x-dayview-header-days';
		els=Ext.query("*[class=" + css+ "]");
		for (var i = 0, len = els.length; i < len; i++) {
			Ext.get(els[i]).setStyle('display', display);
		}
		css = this.id + '-x-monthview-viewer-col-5 x-dayview-header-days';
		els=Ext.query("*[class=" + css+ "]");
		for (var i = 0, len = els.length; i < len; i++) {
			Ext.get(els[i]).setStyle('display', display);
		}
