		...
		for(var i = this.startRow; i < this.endRow; i++){
			...
			for(var j = 0; j < 1; j++){
                ...
                if(0 != rest && i != this.activeStartRow){
	                if(hour >= "08:30" && hour <= "16:30"){
	                	hour = '<div class="x-dayview-viewer-row-height" style="text-align:right;">' +
	                        '<i><b class="x-dayview-lefter-inner x-dayview-lefter-fine-inner"><font color="red">'+hour+'</font></b></i>' +
	                    '</div>';
	                }else{
	                	hour = '<div class="x-dayview-viewer-row-height" style="text-align:right;">' +
	                ...
	                if(hour >= "08:30" && hour <= "17:00"){
	                	hour = '<div class="x-dayview-viewer-row-height">' +
	                        '<b class="x-dayview-lefter-inner"><font color="red">'+hour+'</font></b>' +
	                    '</div>';
	                }else{
	                	hour = '<div class="x-dayview-viewer-row-height">' +
...
//		day: Ext.Date.format(this.daySet[j],this.dayFormat)
		var dayFormatString = "l";
		if(this.dayNum<=3){
			dayFormatString=this.dayFormat;
		}
		for(var j = 0; j < this.dayNum; j++){
			d.push({
                idx:j,
				day: Ext.Date.format(this.daySet[j], dayFormatString)
			});
		}
			...
			'<table width="100%" cellspacing="0" cellpadding="0" border="0"><tbody>' +
				'<tr>' +
					'<td name="'+this.id+'-x-dayview-lefter-width" class="x-dayview-header-lefter">' +
						'<div class="x-dayview-wn">'+week+'周</div>' +
					'</td>' +
...
//                    '<td width="'+this.scrollOffset+'px;">' +
	...
	setToday:function(){      
        ...
        for(var i = 0; i < this.dayNum; i++){
            ...
            if(pbk){
            	...
            	周模式显示不同背景颜色（未完成）
            	if(i < 5){
            		pbk.setStyle('background-color', 'rgb(179,179,255)');
            	}else{
            ...
            for(var j = 0; j < this.dayNum; j++){
                ...
                if(div){
                    ...
                    /*
                        ...
                        td.setStyle('background-color', 'rgb(179,179,255)');
                    }else{
                    	td.setStyle('background-color', 'white');
                    }
                    */
    ...
    endDragEventEl2:function(coverEl, e, lockedValue){
      ...
      var event = coverEl.bindEvent;
       ...
       if('string' == Ext.ux.calendar.Mask.typeOf(event.repeatType)){
          ...
          eh.updateCompleteRepeatEvent(event, cview, oevent);
        }
    },
    ...
    endDragEventEl:function(coverEl, e){
        ...
        ///alert(Ext.ux.calendar.Mask.typeOf(oevent.repeatType));
       /// alert(Ext.ux.calendar.Mask.typeOf(event.repeatType));
...
//              event.repeatType = 'exception';
    ...
    _onPortClickFn:function(e){
        ...
        if(eEl&&eEl.bindEvent){
	        ...
	        if(iniclass(target.className,'x-event-pin')){  
	          ...
	          /*var itemid = "menuitem-" + tgEl.id.substr(7);
	           alert(itemid);
	           var menuitem = Ext.get(itemid);
	           alert(menuitem);*/
	           
	           /* if(iniclass(target.className,'x-calendar-event-pin-off')){
		      		
		      		this.endDragEventEl2(eEl, eEl,true);
				    
				    
	            }else{
		       		//tgEl.removeCls('x-calendar-event-pin-on');
		            //tgEl.addCls('x-calendar-event-pin-off');
				   this.endDragEventEl2(eEl, eEl,false);
	            }*/
	        }else{
	            var cEl;
	...
	_onBodyClickFn:function(e){
        ...
        if(iniclass(target.className,'x-event-detail-tool-close')){
            ...
            if(!eh.readOnly && (iniclass(target.className,'x-whole-title-b') || iniclass(target.className,'x-legend-title-b'))){
                ...
                ///if(wEl && !wEl.bindEvent.locked){
                	 this.fireEvent('canceldetail'); //点击查看前隐藏列表 add by lij
        ...
        bindEl.dragzone = new Ext.dd.DragZone(bindEl, {
            ...
            getDragData:function(e){
                ...
                if(tgEl){
                    ...
                    if(!event.locked && !event.lockedf && event.ctype != "4"){ // modify by lij
...
//            this.leftWidth = 70+r;
    ...
    cleanup:function(col, pool){
	    ...
	    //bug 修正
            for(var n=1;n<tbody.childNodes.length;n++) {
