...
Ext.extend(Ext.ux.calendar.EventHandler, Ext.util.Observable, {
                ...
                if(alertFlag){
                    ...
                    if(0 < alen){
                        for(var j = 0; j < alen; j++){
                            ...
                            if('email' != type){
                                var unit = obj.unit;
                                var early = Ext.ux.calendar.Mask.getMinute(obj.early, unit);
                                var newStartTime = Ext.Date.add(new Date(startTime),Ext.Date.MINUTE, -early);   
                                if(Ext.Date.between(new Date(), newStartTime, endTime)){
                    	...
                    	if(Ext.Date.between(new Date(), startTime, endTime)){
                            expire.push(Ext.applyIf({
                                color:'#'+Ext.ux.calendar.Mask.getColorByIndex(this.calendarSet[event.calendarId].color),
                                startDate:event.day+' '+getHM(this.intervalSlot, event.startRow, this.hourFormat),
                                endDate:endDate
     ...
     * For show/hide a calendar and all events belong to it
     */
    showCanlendarType:function(cid){    	
        var lan = Ext.ux.calendar.Mask.EventHandler;

        var calendar = this.calendarSet[cid];
        ...
        this.ds.createUpdateCalendar(params, function(backObj){
            this.hideCalendar(calendar, false);
//            ext-gen1031-x-calendar-ac86bcf5-3fd1-40bf-b56c-b49187f3f092
//            获取日程分类对象
//            this.showCalendarColor(cEl, calendar.color);

            var mainBody = this.mainPanel.body;
            var ct = Ext.core.DomQuery.select('#'+this.id+'-x-calendar-'+calendar.id, mainBody.dom);
            if(ct){
                for(var i = 0, len = ct.length; i < len; i++){
                    var cEl = Ext.get(ct[i]);
//                    this.changeEventColor(eEl, oldColor, calendar.color);
    ...
    generateInfo:function(event){
        ...
        if(0 == startRow && this.rowCount == endRow){
            ...
            hm = event.day+' '+Ext.ux.calendar.Mask.getIntervalFromRow(this.intervalSlot, startRow, this.hourFormat)+' 至 ';
            if(event.day != event.eday){
                hm += event.eday+' ';
            }
